...
const CRED_PRIV = 'admin_credentials';

// GET /api/employee-actions/credentials
router.get('/credentials', requirePrivilege(CRED_PRIV), async (req, res, next) => {
  try {
    let whereClause = '';
    const params = [];
    if (req.scopedBizIds) {
    ...
    const [rows] = await pool.query(`
      SELECT b.id, b.name, b.afm, b.email, b.city, b.phone,
...
// PUT /api/employee-actions/credentials/:business_id
router.put('/credentials/:business_id', requirePrivilege(CRED_PRIV), async (req, res, next) => {
  try {
    const { business_id } = req.params;
    if (req.scopedBizIds && !req.scopedBizIds.includes(business_id))
      return res.status(403).json({ error: 'Δεν έχετε πρόσβαση σε αυτή την επιχείρηση.' });

    const {
      gsis_username, gsis_password, mydata_user_id, mydata_subscription_key,
      ospa_username, ospa_password,
      timologio_username, timologio_password, timologio_afm,
      epsilon_username, epsilon_password, extra_notes,
    } = req.body;

    await pool.execute(
      `UPDATE businesses SET gsis_username=?, gsis_password=?, mydata_user_id=?, mydata_subscription_key=?, updated_at=NOW() WHERE id=?`,
      [gsis_username||null, gsis_password||null, mydata_user_id||null, mydata_subscription_key||null, business_id]
    );
    await pool.execute(
      `INSERT INTO business_credentials
         (business_id, ospa_username, ospa_password, timologio_username, timologio_password, timologio_afm,
          epsilon_username, epsilon_password, extra_notes, updated_at)
       VALUES (?,?,?,?,?,?,?,?,?,NOW())
       ON DUPLICATE KEY UPDATE
         ospa_username=VALUES(ospa_username), ospa_password=VALUES(ospa_password),
         timologio_username=VALUES(timologio_username), timologio_password=VALUES(timologio_password),
         timologio_afm=VALUES(timologio_afm),
         epsilon_username=VALUES(epsilon_username), epsilon_password=VALUES(epsilon_password),
         extra_notes=VALUES(extra_notes), updated_at=NOW()`,
      [business_id, ospa_username||null, ospa_password||null,
       timologio_username||null, timologio_password||null, timologio_afm||null,
       epsilon_username||null, epsilon_password||null, extra_notes||null]
    );
